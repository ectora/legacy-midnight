...
export default class {
	...
	static verbose2(string: any = 'Empty response', optionalParams: any[] = []) {
		...
		if (typeof string === 'object') {
			return console.log(`${this.date()} ${gradientString('#b1ff1a', '#b1ff3b')(os.userInfo().username.toLowerCase())} ${chalk.gray('VERBOSE ')} ${gradientString('#b1ff3b', '#ff933b')(JSON.stringify(string))}`);
		}

		if (optionalParams && optionalParams.length !== 0) {
			return console.log(`${this.date()} ${gradientString('#b1ff1a', '#b1ff3b')(os.userInfo().username.toLowerCase())} ${chalk.gray('VERBOSE ')} ${gradientString('#b1ff3b', '#ff933b')(string)}`, ...optionalParams);
		}

		return console.log(`${this.date()} ${gradientString('#b1ff1a', '#b1ff3b')(os.userInfo().username.toLowerCase())} ${chalk.gray('VERBOSE ')} ${gradientString('#b1ff3b', '#ff933b')(string)}`);
	}
